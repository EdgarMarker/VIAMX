...
const FilterAndPaginationContent = ({
  ...
  const handleFilterChange = (value: string) => updateUrl({ category: value, page: "1" });
  const goToPage = (num: number) => updateUrl({ page: num.toString() });
  ...
  const renderItem = (item: any, index: number) => {
    switch (variant) {
      ...
      case "productos":
        return <ProductCard key={index} data={item} variant="catalog" />;
      case "productos2":
