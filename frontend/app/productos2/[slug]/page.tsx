...
export async function generateMetadata({ params }: PageProps) {
    ...
    return getPageMetadata(() => getProduct2BySlug(slug));
}

const page = async ({ params }: PageProps) => {
    const { slug } = await params;

    const data = await getProduct2BySlug(slug);
    if (!data) return null;

    const recentProducts = await getProducts2();
    ...
    return (
        <main id="Product2Detail">
            <Hero variant="product2" data={data} />
