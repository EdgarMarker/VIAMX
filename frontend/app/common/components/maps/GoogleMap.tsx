...
export default function GoogleMap({
  ...
  useEffect(() => {
    if (!ready || !mapRef.current || scriptError) return;
    ...
    (async () => {
      ...
      const zoomPos = ControlPosition?.RIGHT_BOTTOM ?? ControlPosition?.RIGHT_CENTER ?? 11;
      ...
      if (mapId) {
        ...
        marker = new AdvancedMarkerElement({ map, position: center, content: img });
      } else {
        marker = new google.maps.Marker({
          map,
          position: center,
          icon: { url: markerSvgUrl, scaledSize: new google.maps.Size(70, 70) },
        });
      }
    })();
  ...
  return (
    <>
      <Script
        ...
        onReady={markReadyWhenAvailable}
        onError={() => setScriptError("Script failed to load")}
      />
      {scriptError ? (
